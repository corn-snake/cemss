...
import { renderActivityCard, renderClases, renderClassCard, renderClassCopyOptions, renderGroupInfo, renderGroupOptions, renderGroups, renderTeacherOptions, renderTermOptions, renderTerms, termFromMonthNumber } from "./queryTransform.js";
...
limit 1`))[0][0];
    return {info, partes, render: renderClassCard(partes, info, template, tema, material)};
},

getActivity = async(uname, template, tema, material)=>{
    try {
        await checkStudent(uname)
    } catch (e) {
        return e;
    }
    const a = (await connection.query(`select idMaterial, titulo, (case when idMaterial=${material} then contenido else false end) as contenido from partes where idPlantilla = "${template}" and idTema = ${tema} and idMaterial between ${material - 1} and ${material + 1}`))[0];
    return renderActivityCard(a, template, tema, material);
};
...
export {validateStaff, validatePupil,
  getStaffFront, getPupilFront, getGroupInfoAdmin, getTeachers, getClassesForCopy, getTerms, getOpenTerms, getGroupsFromTerm, getAllTemplates, getPupilProgress,

  getClassInfo, getActivity,
