...
const getStaffFront = async uname => {
    ...
    const clases = (await connection.query(`select \`clases\`.*, \`grupos\`.nombre AS nomGrupo, \`grupos\`.cuatrimestreActual, \`plantillas\`.nombre as nombre from \`clases\` left join \`grupos\` on \`clases\`.idGrupo = \`grupos\`.idGrupo left join \`plantillas\` on \`plantillas\`.idPlantillas = \`clases\`.idPlantilla where idMaestro = '${uname}' order by \`plantillas\`.nombre asc, \`clases\`.idGrupo asc`))[0];
...
getClassInfo = async (uname, template, tema) => {
    try {
        await tokenCorrect(uname)
    } catch (e) {
        return e;
    ...
    const partes = (await connection.query(`select \`idMaterial\`, \`titulo\` from \`partes\` where \`idPlantilla\` = "${template}"  and idTema = ${tema}`))[0],
        info = (await connection.query(`select count(\`temas\`.idTema) as totalTemas, \`clases\`.\`cambioP12\`, \`clases\`.\`cambioP23\` from \`clases\` left join \`temas\` on \`temas\`.idPlantilla = \`clases\`.idPlantilla where \`clases\`.\`idPlantilla\`="${template}"`))[0][0];
    return {info, partes, };
};
...
const bookResult = async r => {
  ...
  for (const { number,lessons,name } of units) {
    ...
    for (const i in lessons) {
      ...
      try {
        await connection.query(`insert into \`partes\` (idPlantilla, idTema, idMaterial, titulo, contenido) values ("${insertId}", ${number}, ${i}, "${lesname}", "${JSON.stringify(content).slice(1, -1)}")`);
      } catch (e) {
        //console.log(e);
      }
    }
