...
import { renderClases, renderGroups } from "./queryTransform.js";
...
const getStaffFront = async uname => {
    ...
    const clases = (await connection.query(`select * from \`clases\`  where idMaestro = '${uname}' order by nombre asc, idGrupo asc`))[0];
...
\`grupos\`.nombre, \`grupos\`.cuatrimestreActual \
from \`clases\` \
