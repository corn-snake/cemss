...
import { renderActivityCard, renderClases, renderClassCard, renderGroupInfo, renderGroups, renderImparting, renderMarksPersonal, renderTeacherOptions, renderTemplateOptions, renderTermOptions, renderTerms, renderTest, termFromMonthNumber } from "./queryTransform.js";
...
(select max(idMaterial) from partes where idTema = ${tema} and idPlantilla="${template}")`)
    ...
    getMarks = uname => checkStudent(uname).catch(pe=>pe)
        .then(pe=>pe instanceof Error ? pe : connection.query(`select \`alumnos\`.reg_califs,
concat("{\\"", group_concat(CONCAT(\`clases\`.idPlantilla, "\\":\\"", \`plantillas\`.nombre) order by \`clases\`.idPlantilla asc separator "\\",\\""), "\\"}") as listaDeClases,
\`grupos\`.cuatrimestreActual,
CONCAT("{", group_concat(CONCAT("\\"", \`historial_calificaciones\`.cuatrimestre, "\\":", \`historial_calificaciones\`.store) order by \`historial_calificaciones\`.cuatrimestre asc separator ","), "}") as historial
from \`alumnos\`
left join \`clases\` on \`clases\`.idGrupo=\`alumnos\`.idGrupo
left join \`plantillas\` on \`plantillas\`.idPlantilla=\`clases\`.idPlantilla
left join \`grupos\` on \`grupos\`.idGrupo=\`alumnos\`.idGrupo
left join \`historial_calificaciones\` on \`historial_calificaciones\`.idAlumno=\`alumnos\`.idAlumno
where \`alumnos\`.idAlumno="${uname}"
group by \`clases\`.idGrupo, \`historial_calificaciones\`.idAlumno
limit 1`))
        .then(pe => pe instanceof Error ? pe : pe[0][0])
        .then(pe=>pe instanceof Error ? pe : ({marks: pe.historial !== "{}" ? JSON.parse(pe.reg_califs) : {}, classes: typeof pe.listaDeClases !== "null" && `${pe.listaDeClases}`.toUpperCase() !== "NULL" && pe.listaDeClases !== "{}" ? JSON.parse(pe.listaDeClases) : {}, historial: typeof pe.historial !== "null" && `${pe.historial}`.toUpperCase() !== "NULL" && pe.historial !== "{}" ? JSON.parse(pe.historial) : {}, now: pe.cuatrimestreActual}))
        .then(pe => pe instanceof Error ? pe : renderMarksPersonal(pe));
...
export {validateStaff, validatePupil,
  ...
  getClassInfo, getImpartingInfo, getActivity, getUnitExam, getMarks,
